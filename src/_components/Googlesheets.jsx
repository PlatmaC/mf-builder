...
const Googlesheets = ({ optionchanged, createDataSource, options, isSaving, selectedDataSource }) => {
  ...
  function authGoogle() {
    ...
    // clear the old code before start
    localStorage.setItem('OAuthCode', '');
    ...
    datasourceService
      ...
      .then((data) => {
        // TODO !!!!!! change the URL to the appropriate
  ...
  function saveDataSource() {
    ...
    if (code) {
      optionchanged('code', code).then(() => {
        createDataSource();
      });
    } else {
      toast.error('Please complete the authentication first.');
    }
  }
