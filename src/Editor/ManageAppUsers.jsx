...
class ManageAppUsersComponent extends React.Component {
  ...
  toggleAppVisibility = () => {
    ...
    this.setState({ ischangingVisibility: true });
    appService
      .setVisibility(this.state.app.id, newState)
      .then((response) => {
        if (response.status === 403 || response.message === 'Subscription restriction') {
          this.setState({ ischangingVisibility: false });
          if (newState) toast(`You current subscription doesn't allow to make application public.`);
        } else {
          this.setState({ ischangingVisibility: false, app: { ...this.state.app, is_public: newState } });
          if (newState) toast('Application is now public.');
          else toast('Application visibility set to private');
        }
      })
      .catch((error) => {
        this.setState({ ischangingVisibility: false });
  ...
  render() {
    ...
    const appId = app.id;
    // TODO !!!!! There is a problem with the availability of TOOLJET_HOST because the env file didn't apply.
    ...
    const embeddableLink = `<iframe width="560" height="315" src="${shareableLink}" title="Platma app - ${appId}" frameborder="0" allowfullscreen></iframe>`;
                ...
                <div className="make-public mb-3">
                  <div className="form-check form-switch">
                    <input
                      ...
                      checked={Boolean(this.state.app.is_public)}
