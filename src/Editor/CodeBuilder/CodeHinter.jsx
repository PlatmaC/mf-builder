...
import { CustomElements } from './Elements/CustomElements';
...
export function CodeHinter({
  ...
  const slideInStyles = useSpring({
    ...
    to: { opacity: alwaysShowError || isFocused ? 1 : 0, height: alwaysShowError || isFocused ? currentHeight : 0 },
  });
  ...
  const getCustomResolvables = useCallback(() => {
    ...
    return {};
  }, [component?.component?.component, component?.id, fieldMeta?.name, variablesExposedForPreview]);

  const [preview, error] = useMemo(() => {
    if (fieldMeta?.validation?.withErrorMessage && fieldMeta?.validation?.schema.pattern) {
      const [preview, error] = validateByRegExp(
        currentValue,
        fieldMeta?.validation.schema.pattern,
        fieldMeta?.validation.withErrorMessage
      );
      if (error && fieldMeta?.validation?.withErrorMessage) {
        if (!alwaysShowError) {
          setAlwaysShowError(true);
          paramUpdated({ name: paramName, type }, 'isInvalid', true, 'properties');
        }
      } else {
        if (alwaysShowError) {
          setAlwaysShowError(false);
          paramUpdated({ name: paramName, type }, 'isInvalid', false, 'properties');
        }
      }
      return [preview, error];
    } else {
      const customResolvables = getCustomResolvables();
      return resolveReferences(currentValue, currentState, null, customResolvables, true, true);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    alwaysShowError,
  ...
  return (
    <div
      ref={wrapperRef}
      className={cx({
        'codeShow-active': codeShow,
        'd-flex gap-2 justify-content-between': type === 'rundomNumberGenerator',
      })}
    >
        ...
        {paramLabel && (
          <div className={`mb-2 field ${options.className}`} data-cy={`${cyLabel}-widget-parameter-label`}>
            <ToolTip
              ...
              meta={fieldMeta}
              labelClass={`m-0 form-label ${darkMode && 'color-whitish-darkmode'}`}
            />
          </div>
        )}
      ...
      {!codeShow && (
        <div style={{ display: !codeShow ? 'block' : 'none' }}>
          <ElementToRender
            withFxButton={fieldMeta.withFxButton}
