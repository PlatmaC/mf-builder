...
export const Listview = function Listview({
  ...
  function onRowClicked(index) {
    ...
    setExposedVariable('selectedRowInfo', filteredData[index]);
  ...
  useEffect(() => {
    ...
    if (selectedRowIndex != undefined) {
      ...
      setExposedVariable('selectedRowInfo', filteredData[selectedRowIndex]);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [childrenData]);
  ...
  return loadingState ? (
    ...
    <div
      ...
      data-cy={dataCy}
    >
      <div className={`d-flex flex-wrap rows w-100 ${enablePagination && 'pagination-margin-bottom-last-child'}`}>
        {filteredData.map((listItem, index) => (
          <div
            className={`list-item ${mode === 'grid' ? '' : 'w-100'} ${
              showBorder && mode === 'grid' ? '' : 'border-bottom'
            }`}
            style={{
              position: 'relative',
              height: `${rowHeight}px`,
              width: `${100 / (mode === 'grid' ? Number(columns) : 1)}%`,
            }}
            key={index}
          ...
          >
            <SubContainer
              ...
              containerCanvasWidth={width / (mode === 'grid' ? Number(columns) : 1)}
              parent={`${id}`}
