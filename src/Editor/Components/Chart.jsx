...
export const Chart = function Chart({ width, height, darkMode, properties, styles, dataCy }) {
  ...
  const { jsonChartData, chartLayout } = useMemo(() => {
    if (typeof jsonDescription === 'object') {
      return {
        jsonChartData: jsonDescription.data ?? computeChartData(jsonDescription, jsonDescription),
        chartLayout: jsonDescription.layout ?? {},
      };
    } else if (isJson(jsonDescription)) {
      return {
        jsonChartData: JSON5.parse(jsonDescription).data ?? computeChartData(jsonDescription, jsonDescription),
        chartLayout: JSON5.parse(jsonDescription).layout ?? {},
      };
    } else return { jsonChartData: [], chartLayout: {} };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [jsonDescription]);
