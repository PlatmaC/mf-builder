...
export function generateUIComponents(JSONSchema, advanced) {
  if (advanced) {
    ...
    Object.entries(JSONSchema?.properties).forEach(([, value]) => {
      ...
      if (uiComponentsDraft?.length > 0 && uiComponentsDraft[index * 2 + 1]) {
        switch (typeResolver(value?.type)) {
          ...
          case 'DropDown':
            ...
            if (value?.loading)
              uiComponentsDraft[index * 2 + 1]['definition']['properties']['loadingState']['value'] = value?.loading;
            break;
          case 'Button':
            if (value?.styles?.backgroundColor)
              uiComponentsDraft[index * 2 + 1]['definition']['styles']['backgroundColor']['value'] =
    ...
    if (JSONSchema?.submitButton) {
      ...
      if (JSONSchema?.submitButton?.loading)
        uiComponentsDraft[uiComponentsDraft.length - 1]['definition']['properties']['loadingState']['value'] =
