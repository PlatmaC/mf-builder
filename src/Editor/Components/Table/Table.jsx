...
export function Table({
  ...
  const [rowSelectorTouched, setRowSelectorTouched] = useState(false);
  ...
  registerAction(
    ...
    async function (key, value) {
      ...
      const row = rows.find((item) => item.original[key] == value);
      if (row != undefined) {
        setRowSelectorTouched(true);
        ...
        setExposedVariables(selectedRowDetails).then(() => {
          ...
          if (!allowSelection) fireEvent('onRowClicked');
        });
      }
    },
  ...
  useEffect(() => {
    if (rowSelectorTouched) {
      if (showBulkSelector) {
        ...
        const clickedRow = selectedFlatRows?.[0]?.original ?? {};
        const clickedRowId = selectedFlatRows?.[0]?.id ?? null;
        setExposedVariables({ selectedRow, selectedRowId, clickedRow, clickedRowId }).then(() => {
  ...
  const onRowClick = (row) => async (e) => {
    setRowSelectorTouched(true);
    e.stopPropagation();
    if (allowSelection) await toggleRowSelected(row.id);
    const selectedRow = row.original;
    const selectedRowId = row.id;
    setExposedVariables({
      selectedRow: row.isSelected ? selectedRow : {},
      selectedRowId,
      clickedRow: row.original,
      clickedRowId: selectedRowId,
    }).then(() => {
      mergeToTableDetails({ selectedRow, selectedRowId });
      if (!allowSelection) fireEvent('onRowClicked');
    });
