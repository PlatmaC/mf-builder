...
import { urlRegExp } from '@/_helpers/constants';
...
export const PDF = React.memo(({ styles, properties, width, height, component, dataCy }) => {
  ...
  const { url, scale, pageControls, showDownloadOption, renderTextLayer } = properties;
  ...
  const modifiedURL = useMemo(() => {
    const isURL = urlRegExp.test(url);
    if (isURL) return url;
    else if (!url.startsWith('data:')) return `data:application/pdf;base64,${url}`;
    else return url;
  }, [url]);
          ...
          >
            ...
            {showDownloadOption && (
              <div
                ...
                onClick={() => downloadFile(modifiedURL, pdfName)}
              >
                <img
