...
import { MultiInspector } from './Components/MultiInspector/MultiInspector';
...
import { setMockAppId } from '../_helpers/mockAppId';
...
class EditorComponent extends React.Component {
  ...
  setWindowTitle(_name) {
    // document.title = _name ? `${_name} - Platma` : `My App - Platma Editor`;
  }
  ...
  componentDidMount() {
    ...
    // Most probably it's not necessary because it can save the current app until it's loaded.
    // In fact this can save an empty app.
    // this.autoSave();
  ...
  toggleAppMaintenance = () => {
    ...
    appService.setMaintenance(this.state.app.id, newState).then(() => {
      this.setState({
        app: {
          ...this.state.app,
  ...
  fetchApp = (startingPageHandle) => {
    ...
    const callBack = async (data) => {
      let dataDefinition = defaults(data.body, this.defaultDefinition);
  ...
  removeComponent = (component) => {
    ...
    if (!this.props.isVersionReleased) {
      ...
      if (
        newDefinition.pages[currentPageId].components?.[component.id].component.component === 'Tabs' ||
        newDefinition.pages[currentPageId].components?.[component.id].component.component === 'Catalog'
      ) {
  ...
  setSelectedComponent = (id, component, multiSelect = false) => {
    if (multiSelect) {
      this.switchSidebarTab(3);
    } else if (this.state.selectedComponents.length === 0 || !multiSelect) {
  ...
  //   appService
  //     .saveApp(this.state.appId, { definition: this.state.appDefinition })
  //     .then(() => {
  //       this.setState(
  //         {
  //           saveError: false,
  ...
  // TODO !!!!!!!!! This method was changed see above
  saveEditingVersion = (isUserSwitchedVersion = false) => {
          ...
          if (newParentId) {
            ...
            const oldParentId = Object.keys(oldToNewIdMapping).find(
              (parentId) =>
                ...
                ['Tabs', 'Catalog', 'Calendar'].includes(currentPage?.components[parentId]?.component?.component)
            );
  ...
  getCanvasAreaElement = (node) => useAppGlobalStore.getState().actions.setCanvasAreaElement(node);
  ...
  render() {
    ...
    if (process.env.NODE_ENV === 'development') window.state = this.state;
    const switchSidebarTab = this.switchSidebarTab;
    const editingVersion = this.props?.editingVersion;
    // const appVersionPreviewLink = `/applications/${app.id}/${this.state.currentState.page.handle}`;
            ...
            <div className="sub-section">
              ...
              >
                ...
                <QueryPanel
                  ...
                  dataQueriesChanged={this.dataQueriesChanged}
                  refreshPointer={this.props.refreshPointer}
              ...
              <div className="editor-sidebar">
                <div className="editor-sidebar-wrapper">
                    ...
                    <div className="pages-container">
                      <ErrorBoundary
                        fallbackRender={(err) => (
                          <p className="m-3 d-flex flex-column">
                            <span>Some of your widgets have an error:</span>
                      ...
                      >
                        {selectedComponents.length === 1 &&
                          ...
                          !isEmpty(
                            appDefinition.pages[this.state.currentPageId]?.components[selectedComponents[0].id]
                          ) && (
                            <Inspector
                              moveComponents={this.moveComponents}
                              componentDefinitionChanged={this.componentDefinitionChanged}
                  ...
                  {currentSidebarTab === 3 && (
                    <MultiInspector
                      appDefinitionChanged={this.appDefinitionChanged}
                      {...{ currentPageId, currentLayout, selectedComponents, appDefinition, switchSidebarTab }}
                    />
                  )}
                </div>
              </div>
...
const withStore = (Component) => (props) => {
  const { isVersionReleased, editingVersion } = useAppVersionStore((state) => {
    return {
