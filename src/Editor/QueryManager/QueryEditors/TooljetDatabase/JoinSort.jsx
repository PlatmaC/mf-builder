...
export default function JoinSort({ darkMode }) {
  const { tableInfo, joinOrderByOptions, setJoinOrderByOptions, joinOptions } = useContext(TooljetDatabaseContext);
      ...
      const tableDetailsForDropDown = {
        ...
        options:
          tableInfo[table_name]?.map((columns) => ({
            ...
            value: columns.Header + '_' + table_name,
            table: table_name,
          })) || [],
      };
    ...
    <Container fluid className="p-0">
      {isEmpty(joinOrderByOptions) ? (
        ...
        joinOrderByOptions.map((options, i) => {
          const tableDetails = options?.table ? options?.table : '';
          return (
              ...
              <Col sm="6" className="p-0">
                <DropDownSelect
                  ...
                  value={{
                    ...
                    label: tableDetails ? tableDetails + '.' + options.columnName : options.columnName,
                    table: options.table,
                  }}
                  onChange={(option) => {
