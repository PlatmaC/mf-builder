...
export const JoinTable = React.memo(({ currentState, darkMode }) => {
  return (
    <div>
      <SelectTableMenu currentState={currentState} darkMode={darkMode} />
    </div>
  );
});

const SelectTableMenu = ({ currentState, darkMode }) => {
  const {
    selectedTableId,
    joinOptions,
    ...
    <div>
      ...
      <div className="tdb-join-filtersection field-container d-flex" style={{ marginBottom: '1.5rem' }}>
        ...
        <div className="field flex-grow-1 overflow-hidden">
          <RenderFilterSection currentState={currentState} darkMode={darkMode} />
        </div>
      </div>
      {/* Sort Section */}
      <div className="field-container d-flex" style={{ marginBottom: '1.5rem' }}>
...
const RenderFilterSection = ({ currentState, darkMode }) => {
  ...
  tables.forEach((table_name) => {
    if (tableInfo[table_name]) {
      const tableDetailsForDropDown = {
        ...
        value: table_name,
        options:
          tableInfo[table_name]?.map((columns) => ({
            label: columns.Header,
            value: columns.Header + '-' + table_name,
            table: table_name,
          })) || [],
      };
  ...
  return (
    <Container fluid className="p-0">
      ...
      {conditionsList.map((conditionDetail, index) => {
        ...
        const LeftSideTableDetails = leftField?.table ? leftField?.table : '';
        return (
          <Row className="mb-2 mx-0" key={index}>
            ...
            <Col sm="3" className="p-0">
              <DropDownSelect
                ...
                value={{
                  label: LeftSideTableDetails
                    ? LeftSideTableDetails + '.' + leftField?.columnName
                    : leftField?.columnName,
                  value:
                    leftField?.columnName && leftField?.table ? leftField?.columnName + '-' + leftField?.table : '',
