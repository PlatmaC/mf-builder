  ...
  (
    ...
    */
    ...
    const changeDataSource = (source) => {
      ...
      const newQueryName = computeQueryName(source.kind === 'tooljetdb' ? 'inbuilddb' : source.kind);
    ...
    const validateNewOptions = (newOptions) => {
      ...
      if (selectedQuery) {
        ...
        } else if (['restapi', 'platmaapi'].includes(selectedQuery?.kind)) {
          if (headersChanged) {
            isFieldsChanged = true;
          }
        }
    ...
    const renderCustomToggle = ({ dataCy, action, translatedLabel, label }, index) => (
      <div className={cx('mx-4', { 'pb-3 pt-3': index === 1 })} key={index}>
        <CustomToggleSwitch
          dataCy={dataCy}
