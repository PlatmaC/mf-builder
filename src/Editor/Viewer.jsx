...
class ViewerComponent extends Component {
  ...
  setStateForApp = (data) => {
    const copyDefinition = _.cloneDeep(data.body);
  ...
  setStateForContainer = async (data) => {
    ...
    if (this.state.currentLayout === 'mobile') {
      const currentComponents = data.body.pages[data.body.homePageId].components;
    ...
    const pages = Object.entries(data.body.pages).map(([pageId, page]) => ({
      id: pageId,
      ...page,
    }));

    const homePageId = data.body.homePageId;
    ...
    this.setState(
      ...
      () => {
        computeComponentState(this, data?.body?.pages[currentPage.id]?.components).then(async () => {
  ...
  loadApplicationByVersion = (appId, versionId) => {
    appService
      ...
      .then((data) => {
        this.setStateForApp(data.app);
        this.setStateForContainer(data.app);
      })
      .catch((error) => {
        console.error(error);
        this.setState({
  ...
  setupViewer() {
    ...
    if (versionId) {
      ...
      this.loadApplicationBySlug(appId);
    }
  ...
  componentDidMount() {
    useAppGlobalStore.getState().actions.changeIsEditorMode(false);
    this.setupViewer();
  }
  ...
  switchPage = (id, queryParams = []) => {
    ...
    const slug = this.state.slug || this.state.appId;
    const link = `/applications/${slug}/${handle}?${queryParamsString}`;
          ...
          <div className="viewer wrapper">
            ...
            <DndProvider backend={HTML5Backend}>
              ...
              <div className="sub-section">
                <div className="main">
                  <div
                    className={classNames('canvas-container align-items-center', {
                      ['without-scroll-bar']: this.state.currentLayout === 'mobile',
                    })}
                  >
