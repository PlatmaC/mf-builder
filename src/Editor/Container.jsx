...
export const Container = ({
  ...
  const styles = {
    ...
    position: 'absolute',
    backgroundSize: `${canvasWidth / canvasColumnAmount}px 10px`,
  };
  ...
  useHotkeys(
    'meta+v, control+v',
    () => {
      if (isContainerFocused && !isVersionReleased && navigator.clipboard.readText) {
  ...
  function onResizeStop(id, e, direction, ref, d, position) {
    ...
    width = Math.round(width + (deltaWidth * canvasColumnAmount) / canvasWidth); // convert the width delta to percentage
        ...
        if (!box.parent && resolveReferences(canShowInCurrentLayout, currentState)) {
          return (
            <DraggableBox
              ...
              onComponentClick={config.COMMENT_FEATURE_ENABLE && showComments && handleAddThreadOnComponent}
              onEvent={onEvent}
