...
export const Inspector = ({
  ...
  if (!componentMeta) {
    return (
      <p className="m-3 d-flex flex-column">
        ...
        <span>{`${component.component.component} doesn't have meta info`}</span>
      </p>
    );
  }
  ...
  const multyParamsUpdater = (paramsArray) => {
    let newDefinition;
    paramsArray.forEach(({ param, attr, value, paramType }) => {
      newDefinition = newDefinitionGenerator(param, attr, value, paramType, newDefinition);
    });
    componentDefinitionChanged(_.merge(component, { component: { definition: newDefinition } }));
  };

  // temporary copy-pasted from the paramsUpdated function to avoid unexpected bugs or side affects
  // due to insufficient understanding of the code
  const newDefinitionGenerator = (param, attr, value, paramType, previousDefinition) => {
    let newDefinition = previousDefinition ?? _.cloneDeep(component.component.definition);
  ...
  function paramUpdated(param, attr, value, paramType, isMerge = true) {
    ...
    newDefinition[paramType] = allParams;
    let newComponent;
    if (isMerge) newComponent = _.merge(component, { component: { definition: newDefinition } });
    else newComponent = _.set(component, 'component.definition', newDefinition);
    componentDefinitionChanged(newComponent);
  }
  ...
  return (
    <div className="inspector">
      ...
      {!componentMeta.withoutDocumentationLink && (
        <div className="widget-documentation-link p-2">
          ...
          >
            <small>
              {t('widget.common.documentation', '{{componentMeta}} documentation', {
...
GetAccordion.displayName = 'GetAccordion';
