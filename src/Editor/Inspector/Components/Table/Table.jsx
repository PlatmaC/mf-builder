...
import { convertDataTypeToColumnType } from '@/Editor/Components/Table/columns/autogenerateColumns';
...
class TableComponent extends React.Component {
  ...
  generateColumns = () => {
    const tableData = this.props.componentMeta.definition.properties.data.value
      ? resolveReferences(this.props.component.component.definition.properties.data.value, this.props.currentState, [])
      : [];

    if (Array.isArray(tableData) && tableData[0]) {
      const newColumns = Object.entries(tableData[0]).map(([key, value]) => ({
        columnType: convertDataTypeToColumnType(typeof value),
        autogenerated: true,
        id: uuidv4(),
        name: key,
        key: key,
      }));
      this.props.paramUpdated({ name: 'columns' }, 'value', newColumns, 'properties', false);
    } else this.props.paramUpdated({ name: 'columns' }, 'value', [], 'properties', false);
  };
    ...
    items.push({
      ...
      children: (
        <div>
          {!useDynamicColumn && (
            <>
              <div className="d-flex gap-2 col-auto text-right mb-3">
                <Button className="btn-sm" onClick={this.generateColumns}>
                  Generate from Data
                </Button>
                <button
