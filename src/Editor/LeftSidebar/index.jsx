...
export const LeftSidebar = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    const newError = _.flow([
      ...
      (arr) => arr.filter(([, value]) => value.data?.status),
      Object.fromEntries,
    ])(errors);
    ...
    if (newErrorLogs) {
      setErrorLogs((prevErrors) => {
        const copy = JSON.parse(JSON.stringify(prevErrors, getCircularReplacer()));
  ...
  useEffect(() => {
    if (open) {
      // eslint-disable-next-line no-unused-vars
      setUnReadErrorCount(() => ({ read: errorLogs.length, unread: 0 }));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open]);
