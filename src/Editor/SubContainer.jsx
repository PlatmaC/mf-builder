...
export const SubContainer = ({
  ...
  useEffect(() => {
    if (mounted) {
      ...
      if (children.length === 0 && addDefaultChildren === true) {
        ...
        const childrenBoxes = {};
        const parentId =
          parentComponent.component !== 'Tabs' && parentComponent.component !== 'Catalog'
        ...
        defaultChildren.forEach((child) => {
          ...
          if (_.isArray(others) && others.length > 0) {
            others.forEach((prop) => {
              const accessor = defaultValue[prop] || '';
              _.set(newComponentDefinition, prop, { value: accessor });
            });
            _.set(componentData, 'definition.others', newComponentDefinition);
          }
          ...
          if (_.isArray(events) && events.length > 0) {
            const modifiedEvents = events.map((event) => {
              if (event.componentId === 'parent') event.componentId = parentId;
              if (event.componentId === 'self') event.componentId = newComponent.id;
              if (_.isArray(event.componentSpecificActionParams) && event.componentSpecificActionParams.length > 0) {
                event.componentSpecificActionParams = event.componentSpecificActionParams.map((actionParam) => {
                  if (actionParam.value.includes('parent'))
                    actionParam.value = actionParam.value.replace('parent', parentComponent.name);
                  if (actionParam.value.includes('self'))
                    actionParam.value = actionParam.value.replace('self', newComponent.component.name);
                  return actionParam;
                });
              }
              return event;
            });
            _.set(newComponent, 'component.definition.events', modifiedEvents);
          }
          ...
          _.set(childrenBoxes, newComponent.id, {
            ...
            withDefaultChildren: Boolean(withDefaultChildren),
          });
        });
  ...
  function onResizeStop(id, e, direction, ref, d, position) {
    ...
    width = width + (deltaWidth * canvasColumnAmount) / subContainerWidth;
