...
class DataSourceManagerComponent extends React.Component {
  ...
  getSelectedDataSource = () => {
    const selectedDataSource = this.props.selectedDataSource || null;
    const options = selectedDataSource?.options || {};

    return {
      name: selectedDataSource?.name,
      selectedDataSource,
      options,
      dataSourceMeta: selectedDataSource ? this.getDataSourceMeta(selectedDataSource) : {},
      dataSourceSchema: selectedDataSource?.plugin?.manifestFile?.data || null,
      selectedDataSourceIcon: selectedDataSource?.plugin?.iconFile.data || null,
    };
  };
  ...
  selectDataSource = (source) => {
    this.props.setSelectedDataSource(source);
    // this.setState({
  ...
  onNameChanged = (newName) => {
    ...
    this.props.setSelectedDataSource({ ...selectedDataSource, name: newName });
  };

  onChangeOptions = (options) => {
    const { selectedDataSource } = this.getSelectedDataSource();
    this.props.setSelectedDataSource({ ...selectedDataSource, options });
  };
  ...
  optionchanged = (option, value) => {
    return new Promise((resolve) => {
      const { selectedDataSource, options } = this.getSelectedDataSource();
      this.props.setSelectedDataSource({
        ...selectedDataSource,
        options: { ...options, [option]: { ...options[option], value } },
      });

      setTimeout(() => {
        resolve(this.getSelectedDataSource());
      });
    });
  };
  ...
  createDataSource = () => {
    ...
    const { options, selectedDataSource, selectedDataSourcePluginId } = this.getSelectedDataSource();
    ...
    const parsedOptions = Object.keys(options)
      .map((key) => {
        ...
        return {
          ...
          encrypted: keyMeta && keyMeta.encrypted ? keyMeta.encrypted : false,
  ...
  renderSourceComponent = (kind, isPlugin = false) => {
    const { options, dataSourceSchema, selectedDataSource } = this.getSelectedDataSource();
    ...
    return (
      <ComponentToRender
        ...
        isEditMode={!isEmpty(selectedDataSource)}
      />
    );
  };
  ...
  segregateDataSources = (suggestingDatasources) => {
    ...
    return (
      ...
      //     </Col>
      //     <Col className={`modal-body-content ${darkMode ? 'dark' : ''}`}>
    ...
    const dataSourceList = [
      {
        ...
        key: '#alldatasources',
        list: PlatmaSupportedSources,
        renderDatasources: () => this.renderCardGroup(PlatmaSupportedSources),
      },
    ];
  ...
  render() {
    const { isSaving, connectionTestError, isCopied } = this.state;

    const { dataSourceMeta, selectedDataSource, options, dataSourceSchema, selectedDataSourceIcon } =
      this.getSelectedDataSource();
    const isPlugin = dataSourceSchema ? true : false;
    return selectedDataSource ? (
      <div className="datasource-body mt-4">
        <div className="header">
          <div
            className={`back-btn me-3 mt-1 ${this.props.darkMode ? 'dark' : ''}`}
            role="button"
          ...
          <div className="row selected-ds mt-2">
        ...
        <div className="mt-2">{this.renderSourceComponent(selectedDataSource.kind, isPlugin)}</div>
      ...
      <div className="datasources-available-list">
