...
export const ListItem = ({ active, onClick, text = '', onDeleteCallback, onCloneCallback }) => {
  ...
  const [isCloning, setIsClonign] = useState(false);
  ...
  const handleCloneTable = (tableName) => async () => {
    setIsClonign(true);
    const { error, data } = await tooljetDatabaseService.cloneTable(organizationId, tableName);
    if (error) toast.error(error?.message ?? `Failed to clone table "${tableName}"`);
    else {
      toast.success(data.result.message ?? `Table "${tableName}" cloned successfully`);
      onCloneCallback?.();
    }
    setIsClonign(false);
  };
