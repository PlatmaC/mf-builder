...
import PlatmaApiIcon from '@/Editor/Icons/platmaapi.svg';
...
import { allSvgs } from '@plugins/client';
import {
...
import { simpleEventNames } from './constants';
...
export async function executeActionsForEventId(_ref, eventId, component, mode, customVariables) {
  ...
  for (const event of filteredEvents) {
    const executeEvent = debounce(executeActionWithDebounce);
    await executeEvent(_ref, event, mode, customVariables);
  }
}
...
function createCurrentStateBatching() {
  const storeProcess = new WeakMap();
  const currentStates = new WeakMap();
  const callbacks = new WeakMap();

  const pushItem = (ent, _ref, item) => {
    ent.set(_ref, [...(ent.get(_ref) || []), item]);
  };

  const getItems = (ent, _ref, length) => {
    return [...(ent.get(_ref) || [])].slice(0, length);
  };

  const removeItems = (ent, _ref, length) => {
    return ent.set(_ref, [...(ent.get(_ref) || [])].slice(length));
  };

  const applyState = (_ref) => {
    const callbacksLength = callbacks.get(_ref)?.length | 0;
    const stateLength = currentStates.get(_ref)?.length | 0;

    const currentState = getCurrentState(_ref);
    const newState = {
      ..._ref.state,
      currentState,
    };
    ...
    _ref.setState(newState, () => {
      getItems(callbacks, _ref, callbacksLength).forEach((cb) => {
        cb();
      });
      removeItems(callbacks, _ref, callbacksLength);
      removeItems(currentStates, _ref, stateLength);
    });
  };

  const getCurrentState = (_ref) => {
    const chain = currentStates.get(_ref) || [];
    return chain.length ? chain.at(-1) : _ref.state.currentState;
  };

  const setCurrentState = (_ref, fn, cb = null) => {
    if (cb) {
      pushItem(callbacks, _ref, cb);
    }
    pushItem(currentStates, _ref, fn(getCurrentState(_ref)));

    if (!storeProcess.has(_ref)) {
      storeProcess.set(_ref, true);
      setTimeout(() => {
        storeProcess.delete(_ref);
        applyState(_ref);
      });
    }
    return _ref;
...
const batchedCurrentState = createCurrentStateBatching();
...
function executeActionWithDebounce(_ref, event, mode, customVariables) {
  const currentState = batchedCurrentState.getCurrentState(_ref);
  if (event) {
    if (event.runOnlyIf) {
      const shouldRun = resolveReferences(event.runOnlyIf, currentState, undefined, customVariables);
      if (!shouldRun) {
        return false;
      }
    }
    switch (event.actionId) {
      case 'show-alert': {
        const message = resolveReferences(event.message, currentState, undefined, customVariables);
      ...
      case 'open-webpage': {
        const url = resolveReferences(event.url, currentState, undefined, customVariables);
        window.open(url, '_blank');
        return Promise.resolve();
      }

      case 'go-to-app': {
        const slug = resolveReferences(event.slug, currentState, undefined, customVariables);
        const queryParams = event.queryParams?.reduce(
          (result, queryParam) => ({
            ...result,
            ...{
              [resolveReferences(queryParam[0], currentState)]: resolveReferences(
                queryParam[1],
                currentState,
                undefined,
                customVariables
      ...
      case 'copy-to-clipboard': {
        const contentToCopy = resolveReferences(event.contentToCopy, currentState, undefined, customVariables);
      ...
      case 'set-localstorage-value': {
        const key = resolveReferences(event.key, currentState, undefined, customVariables);
        const value = resolveReferences(event.value, currentState, undefined, customVariables);
      ...
      case 'generate-file': {
        ...
        const data = resolveReferences(event.data, currentState, undefined, customVariables) ?? [];
        const fileName = resolveReferences(event.fileName, currentState, undefined, customVariables) ?? 'data.txt';
        const fileType = resolveReferences(event.fileType, currentState, undefined, customVariables) ?? 'csv';
        const fileData = {
          csv: generateCSV,
      ...
      case 'set-custom-variable': {
        ...
        return batchedCurrentState.setCurrentState(_ref, () => {
          return { ...batchedCurrentState.getCurrentState(_ref), variables };
        });
      }
      ...
      case 'set-page-variable': {
        ...
        return batchedCurrentState.setCurrentState(_ref, (currState) => {
          return { ...batchedCurrentState.getCurrentState(_ref), page: { ...currState.page, variables } };
        });
      }

      case 'unset-page-variable': {
        const key = resolveReferences(event.key, currentState, undefined, customVariables);
        const variables = _.omit(currentState.page.variables, key);
      ...
      case 'control-component': {
        ...
        const actionArguments = _.map(event.componentSpecificActionParams, (param) => ({
          ...param,
          value: resolveReferences(param.value, currentState, undefined, customVariables),
        }));
...
export async function onEvent(_ref, eventName, options, mode = 'edit') {
  ...
  if (simpleEventNames.includes(eventName)) {
...
export function runQuery(_ref, queryId, queryName, confirmed = undefined, mode = 'edit') {
  ...
  const options = getQueryVariables(dataQuery.options, currentState);
  ...
  const newCurrentState = {
    ...
    queries: {
      ...currentState.queries,
      [queryName]: {
        ...currentState.queries[queryName],
        isLoading: true,
        data: [],
        rawData: [],
      },
  ...
  return new Promise(function (resolve) {
    ...
    batchedCurrentState.setCurrentState(
      _self,
      () => newCurrentState,
      () => {
        let queryExecutionPromise = null;
        ...
        queryExecutionPromise
          .then(async (data) => {
            ...
            if (promiseStatus === 'failed' || promiseStatus === 'Bad Request') {
                  ...
                  currentState: {
                    ...
                    queries: {
                      ...
                      [queryName]: _.assign(
                        ...
                        query.kind === 'restapi' || query.kind === 'platmaapi'
                          ? {
              ...
              const currentState = batchedCurrentState.getCurrentState(_self);
...
export function computeComponentState(_ref, components = {}) {
  ...
  Object.keys(components).forEach((key) => {
    ...
    if (!componentMeta) {
      console.error(`Unable to find ${component.component.component} meta information`);
      toast.error(`Unable to find ${component.component.component} meta information. Remove or modify this component.`);
    }
...
export const getSvgIcon = (key, height = 50, width = 50, iconFile = undefined, styles = {}) => {
  ...
  if (key === 'platmaapi') return <PlatmaApiIcon />;
...
export const debuggerActions = {
  ...
  generateErrorLogs: (errors) => {
    ...
    Object.entries(errors).forEach(([key, value]) => {
      const errorType =
        value.type === 'query' &&
        (value.kind === 'restapi' || value.kind === 'platmaapi' || value.kind === 'tooljetdb' || value.kind === 'runjs')
          ? value.kind
          : value.type;
...
export const cloneComponents = (_ref, updateAppDefinition, isCloning = true, isCut = false) => {
  ...
  if (isCloning) {
    ...
    if (navigator.clipboard.readText) {
      ...
      toast.error('This browser does not support writing to the clipboard');
    }
  }
...
const getChildComponents = (allComponents, component, parentComponent, addedComponentId) => {
  ...
  if (
    ...
    component.component.component === 'Catalog' ||
  ...
  childComponents.forEach((componentId) => {
    ...
    if (
      (component.component.component === 'Tabs' || component.component.component === 'Catalog') |
      (component.component.component === 'Calendar')
    ) {
        ...
        if (parentComponent) {
          if (
            ...
            (parentComponent.component.component === 'Tabs' ||
              parentComponent.component.component === 'Catalog' ||
...
export const addNewWidgetToTheEditor = (
  ...
  componentData.definition.others.showOnDesktop.value = true;
  componentData.definition.others.showOnMobile.value = true;

  const widgetsWithDefaultComponents = ['Listview', 'Tabs', 'Form', 'Kanban', 'Catalog'];
...
export const removeSelectedComponent = (pageId, newDefinition, selectedComponents) => {
  selectedComponents.forEach((component) => {
    ...
    if (
      newDefinition.pages[pageId].components[component.id]?.component?.component === 'Tabs' ||
      newDefinition.pages[pageId].components[component.id]?.component?.component === 'Catalog'
    ) {
