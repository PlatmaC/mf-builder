...
export const authenticationService = {
  ...
  get currentSessionValue() {
    return { ...currentSessionSubject.value, current_organization_id: getCookie('orgIdForOauth') || null };
  },
...
function login(email, password, organizationId) {
  ...
  return fetch(`${API_URL}/authenticate${organizationId ? `/${organizationId}` : ''}`, requestOptions)
...
function validateSession(appId) {
  ...
  return fetch(`${API_URL}/session${appId ? `?appId=${appId}` : ''}`, requestOptions).then(
    handleResponseWithoutValidation
  );
}
...
function getOrganizationConfigs(organizationId) {
  ...
  return fetch(`${API_URL}/organizations/${organizationId ? `${organizationId}/` : ''}public-configs`, requestOptions)
    .then(handleResponse)
...
function resendInvite(email) {
  ...
  return fetch(`${API_URL}/resend-invite`, requestOptions)
    .then(handleResponse)
    .then((response) => {
      return response;
    });
...
function onboarding({ companyName, companySize, role, token, organizationToken, source, password, phoneNumber }) {
  ...
  return fetch(`${API_URL}/setup-account-from-token`, requestOptions)
    .then(handleResponse)
    .then((response) => {
      return response;
    });
...
function setupAdmin({ companyName, companySize, name, role, workspace, password, email, phoneNumber }) {
  ...
  return fetch(`${API_URL}/setup-admin`, requestOptions)
    .then(handleResponse)
    .then((response) => {
      return response;
    });
...
function verifyOrganizationToken(token) {
  ...
  return fetch(`${API_URL}/verify-organization-token?token=${token}`, requestOptions)
    .then(handleResponse)
    .then((response) => {
      return response;
    });
...
function verifyToken(token, organizationToken) {
  ...
  return fetch(
    `${API_URL}/verify-invite-token?token=${token}${
...
function resetPassword(params) {
  ...
  return fetch(`${API_URL}/reset-password`, requestOptions).then(handleResponse);
}
...
function logout(avoidRedirection = false) {
  ...
  return fetch(`${API_URL}/authorization/logout`, requestOptions)
    .then(handleResponseWithoutValidation)
    .then(() => {
...
function signInViaOAuth(configId, ssoType, ssoResponse) {
  ...
  return fetch(`${API_URL}/oauth/sign-in/${url}`, requestOptions)
    .then(handleResponseWithoutValidation)
    .then((user) => {
...
function authorize() {
  ...
  return fetch(`${API_URL}/authorize`, requestOptions).then(handleResponseWithoutValidation);
}
