...
function getAll() {
  ...
  return fetch(`${API_URL}/v2/data_sources`, requestOptions).then(handleResponse);
}
...
function save({ id, name, options, environment_id }) {
  ...
  return fetch(`${API_URL}/v2/data_sources/${id}?environment_id=${environment_id}`, requestOptions).then(
    handleResponse
  );
}
...
function deleteDataSource(id) {
  ...
  return fetch(`${API_URL}/v2/data_sources/${id}`, requestOptions).then(handleResponse);
}

function convertToGlobal(id) {
  const requestOptions = { method: 'POST', headers: authHeader(), credentials: 'include' };
  return fetch(`${API_URL}/v2/data_sources/${id}/scope`, requestOptions).then(handleResponse);
}
